"""
03_tfidf_representation.py
==========================
Sparse representation 의 두 주류인 BoW (raw count) 와 TF-IDF 를 비교한다.

학습 목표
---------
1. IDF 가 "여러 문서에 흔하게 등장하는 단어" 의 가중치를 효과적으로 낮춘다.
2. 분류기 관점에서 TF-IDF 가 BoW 보다 유리한 상황을 확인.
3. "Sparse" 의 의미: 문서당 평균 nnz / 전체 vocabulary 크기 시각화.

실험 설정
---------
- 강의 예시 스타일의 합성 corpus (긍/부정 + 공통 stop-like 단어)
  → 의도적으로 공통어가 많이 섞이게 해서 IDF 효과가 드러나게 함.
- Multinomial NB 에 BoW 와 TF-IDF 특징을 넣어 정확도 비교.
- 어휘별 IDF 값을 시각화해서 stop word 가 자동으로 억제되는지 확인.

실행: python scripts/03_tfidf_representation.py
결과: figures/03_tfidf_representation.png
"""
...
# -------------------------------------------------------------
# Corpus: 긍/부정 신호 + 대량의 stop-like 공통어 섞기
# -------------------------------------------------------------
...
# "the, is, on" 류 stop-like: 두 클래스에 고르게, 그러나 문서 대부분을 차지하도록.
# 이 상황에서 BoW (raw count) 를 LR 에 넣으면 stop-like 단어의 큰 카운트가
# gradient 에 잡음을 주지만, TF-IDF 는 낮은 IDF 덕에 자동으로 억제된다.
STOP_LIKE = ["the", "is", "on", "of", "and", "a", "to", "this", "that",
             "in", "for", "with", "at", "by", "an"]
COMMON = ["movie", "film", "was", "i", "it", "really", "quite"]

# 감성 단어는 매우 적게 (topic 신호 약화), stop-like 비중 극단적으로 크게
N_TOPIC = (1, 3)
N_COMMON = (2, 5)
N_STOP = (40, 60)
...
# -------------------------------------------------------------
# 실험: BoW vs TF-IDF × 여러 분류기
# -------------------------------------------------------------
def run_experiment(n_runs=30, n_docs=60):
    """n_docs 가 매우 작아서 감성 단어가 우연히 편향되면 BoW LR 은
    stop-like 의 큰 raw count 에 가중치를 맞춰버려 일반화가 잘 안 된다."""
    rows = []  # (repr, model, acc)
...
def compute_idf_spectrum(seed=0, n_docs=200):
    ...
    # sklearn idf_ = log((1+N)/(1+df)) + 1  (smoothing 포함)
    idfs = vec.idf_
    # 단어를 그룹별로 분류
    def group_of(w):
        if w in STOP_LIKE:
...
def plot(rows, summary):
    ...
    # (b) 정확도 비교 (BoW vs TF-IDF × NB vs LR)
    ax = axes[1]
