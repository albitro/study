...
> 🧪 이 계산을 NumPy scratch 로 그대로 재현한다: [`scripts/01_naive_bayes_scratch.py`](scripts/01_naive_bayes_scratch.py).
> 스크립트 결과: $P(-) \cdot P(S \mid -) = 6.106 \times 10^{-5}$, $P(+) \cdot P(S \mid +) = 3.280 \times 10^{-5}$ — 강의 수치와 정확히 일치.
...
MLE 추정은 "한 번도 못 본" 조합에 대해 확률 0 을 부여한다. 예를 들어 학습 데이터에 `('fantastic', Positive)` 조합이 없으면:
...
분류 정확도는 $\alpha$ 에 대해 **U-shape** 를 그린다: 너무 작으면 zero-prob 에 당하고, 너무 크면 사전 정보가 균등분포를 과도하게 주입해 데이터가 주는 신호를 덮어 버린다. 표준 선택 $\alpha = 1$ 이 합리적인 지점임을 경험적으로 확인할 수 있다.

> 🧪 [`scripts/02_smoothing_effect.py`](scripts/02_smoothing_effect.py) 에서 20 회 반복 평균 + std.
...
각 차원(= feature) 은 어떤 단어 또는 규칙을 나타내며, 대부분의 차원은 0 이 되는 게 일반적이다 — 이 때문에 **sparse** 라고 부른다.
...
→ $\mathbf{x} = [3, 2, 0, 2, 20, 0]$. NB 의 likelihood 는 이 경우에도 그대로 확장된다:
...
단어 수준에서 "**이 문서에서 얼마나 자주 나오는가**(TF)" 만 보는 BoW 에는 흠이 있다 — 'the', 'is' 같은 단어도 높은 빈도로 등장하지만 문서 특성을 잘 드러내는 단어는 아니다. TF-IDF 는 이 문제를 **문서 간 분포**를 보는 IDF 로 완화한다.
...
3 개 문서 $D_1 =$ "cat sits on the mat", $D_2 =$ "dog barks at the cat", $D_3 =$ "mouse runs under the mat" 에서:
...
1. **LR + TF-IDF** (0.746) 가 **LR + BoW** (0.622) 대비 +12%p 향상 — TF-IDF 가 stop-like 단어의 영향을 억제해 gradient 에 잡음이 덜 섞인다.
2. **NB + TF-IDF** 는 오히려 불리 — Multinomial NB 는 정수 count 에 대한 generative model 이라 TF-IDF 의 실수 값에 맞지 않는다. 강의에선 다루지 않지만 실무적으로 중요한 포인트: **표현(feature)과 모델은 궁합이 있다**.

IDF 스펙트럼 시각화에서 실제로 stop-like 단어들은 모두 1.0 근처, 감성 단어들은 3~4 로 확연히 구분됨을 확인할 수 있다.

> 🧪 [`scripts/03_tfidf_representation.py`](scripts/03_tfidf_representation.py).
...
> 🧪 [`scripts/04_logistic_regression_scratch.py`](scripts/04_logistic_regression_scratch.py) Part B 에서 sanity check. 실제 계산 결과 $\sigma(0.833) = 0.697$ — 강의 수치와 일치.
...
Negative log-likelihood 를 loss 로 사용:
...
형태가 매우 간결하다 — "예측 오차 $(\hat{y} - y)$ × 입력" 의 합.
...
LR 의 CE loss 는 $\theta$ 에 대해 **convex** 이므로 gradient descent 가 전역 최솟값을 찾는다.

> 🧪 [`scripts/04_logistic_regression_scratch.py`](scripts/04_logistic_regression_scratch.py) Part A — 2D toy 데이터에서 loss 가 단조 감소하며 수렴 (final loss = 0.203, train acc = 0.920).
...
학습 데이터에 완벽히 맞추려다 noise 까지 외워버리면 새 데이터에서 성능이 떨어진다. 특히 **데이터가 적고 feature 차원이 큰** 경우, LR 은 우연한 상관에 강한 가중치를 부여해버리기 쉽다.
...
고차원 합성 데이터 ($n = 30$ 학습, $d = 80$) 에서 $\lambda$ 를 바꿔가며 학습한 결과:


| $\lambda$ | Train Acc | Test Acc | $\|\mathbf{w}\|_2$ |
| ----------: | :---------: | :--------: | :------------------: |
|       $0$ |   1.000   |  0.620  |      **4.31**      |
|    $0.01$ |   1.000   |  0.614  |        2.60        |
|     $0.1$ |   1.000   |  0.608  |        1.41        |
|       $1$ |   0.986   |  0.589  |        0.48        |
|       $5$ |   0.820   |  0.550  |      **0.15**      |

핵심 관찰: **weight norm 이 28배 감소** ($4.31 \to 0.15$). 이게 L2 가 하는 일의 본질이다. Linearly separable 데이터에서 $\lambda = 0$ LR 은 weight 가 계속 커지며 수렴하는데, L2 는 이를 유한하게 묶어 모델 복잡도를 제어한다.
...
$m$ 개 클래스 $\mathcal{C} = \{1, \dots, m\}$ 에 대해 sigmoid 를 **softmax** 로 일반화한다:
...
출력은 총합 1 인 확률 분포를 이룬다.

### 8.2 Multinomial CE Loss 와 Gradient
...
Binary 경우와 마찬가지로 "정답 - 예측" × "입력" 의 형태.

### 8.3 Feature design

Multinomial LR 에서는 feature 를 "입력만의 함수" $f(x)$ 가 아닌, **"입력 × 클래스" 의 함수** $f(c, x)$ 로 디자인해 각 클래스마다 다른 가중치를 부여할 수 있다.
...
같은 "`!` 가 있음" 이라는 feature 가 클래스마다 다른 weight 를 갖는다.
...
| 큰 데이터     | 독립 가정의 bias 에 묶임             | 더 유연하게 fit, 성능 역전 가능 |
| Feature 상관  | 취약 (독립 가정이 깨짐)              | 강건                            |
| 속도          | 매우 빠름                            | 상대적으로 느림 (반복 최적화)   |

**언제 무엇을 쓸까?**

- 작은 학습 데이터 (수십~수백 문서), 빠른 baseline 이 필요한 경우 → **Naive Bayes**.
- 충분한 데이터가 있고 feature 간 상관이 있을 수 있는 경우, 또는 TF-IDF 같은 연속값 feature 를 쓸 경우 → **Logistic Regression**.

LR 은 신경망의 출발점이기도 하다 — softmax 출력층 + CE loss 는 사실상 그대로 multinomial LR 이다.
...
| 번호 | 파일                                                                                     | 내용                                                                                               | 스타일        | 핵심 수치                                                                                                                      |
| :----: | ------------------------------------------------------------------------------------------ | ---------------------------------------------------------------------------------------------------- | --------------- | -------------------------------------------------------------------------------------------------------------------------------- |
|  01  | [`scripts/01_naive_bayes_scratch.py`](scripts/01_naive_bayes_scratch.py)                 | 강의 worked example ("predictable no fun") 완전 재현 + 토큰별 log-likelihood 기여도 시각화         | NumPy scratch | $P(-)\cdot P(S\mid-)=6.11\times 10^{-5}$, $P(+)\cdot P(S\mid+)=3.28\times 10^{-5}$, 예측 = `-`                                 |
|  02  | [`scripts/02_smoothing_effect.py`](scripts/02_smoothing_effect.py)                       | Laplace$\alpha$ 스윕 (20 회 반복)                                                                  | NumPy scratch | $\alpha=0$ → 0.730, $\alpha=1$ → **0.917**, $\alpha=10^3$ → 0.535 (U-shape)                                                 |
|  03  | [`scripts/03_tfidf_representation.py`](scripts/03_tfidf_representation.py)               | BoW vs TF-IDF × NB vs LR 비교 + IDF 스펙트럼                                                      | scikit-learn  | LR: BoW 0.622 → TF-IDF**0.746** (+0.12); NB: BoW 0.843 → TF-IDF 0.654                                                        |
|  04  | [`scripts/04_logistic_regression_scratch.py`](scripts/04_logistic_regression_scratch.py) | Sigmoid + CE + gradient descent 직접 구현. (A) 2D decision boundary (B) 강의 예제 재현 (C) L2 스윕 | NumPy scratch | $\sigma(0.833)=0.697\approx 0.69$; L2 $\lambda=0$ → $\|\mathbf{w}\|=4.31$, $\lambda=5$ → $\|\mathbf{w}\|=0.15$ (28× shrink) |

### 재현 방법

```bash
pip install -r requirements.txt
python scripts/01_naive_bayes_scratch.py
python scripts/02_smoothing_effect.py
python scripts/03_tfidf_representation.py
python scripts/04_logistic_regression_scratch.py
```

모든 그림은 `figures/` 폴더에 PNG 로 저장된다.

---

## 참고

- 원강의 자료: Text Mining (BDC102), Korea University — Buru Chang 교수님.
- Jurafsky & Martin, *Speech and Language Processing* (3rd ed. draft), Ch. 4 (Naive Bayes), Ch. 5 (Logistic Regression).
- Ng, A. & Jordan, M. (2001). *On discriminative vs. generative classifiers: A comparison of logistic regression and naive Bayes*. NeurIPS — NB vs LR 의 asymptotic trade-off 를 수학적으로 분석한 고전.
